...
class Trainer_NoSSL(BaseTrainer):
    def __init__(self, model, config, use_dice=True):
        super(Trainer_NoSSL, self).__init__(model, config)    
        #if True, using skel_target as the weigts for calculating reg loss  
        self.use_dice = use_dice 
       
    def start_train(self, train_dataset, test_dataset, save_model_dir, pretrained_file=None):
        '''
        Input:
        ...
        with self.summary_writer.as_default(): 
            ...
            for self.epoch in range(self.cfg.EPOCHS):
                ...
                #training an epoch
                t1 = time.time()
                agv_ske_loss, agv_seg_ce_loss, agv_seg_dice_loss, agv_reg_loss = self.__train_epoch(train_dataset, optimizer)
                t2 = time.time()
                current_lr = KB.eval(optimizer._decayed_lr('float32'))
                
               
                print ('\nSke loss: %f; Seg ce loss: %f; Seg dice loss: %f; Reg loss: %f; Lr: %f; Used time (s): %f' % \
                      (agv_ske_loss, agv_seg_ce_loss, agv_seg_dice_loss, agv_reg_loss, current_lr, t2-t1)) 
    ...
    def __train_epoch(self, train_dataset, optimizer):
        ...
        for step in range(self.cfg.STEPS_PER_EPOCH):
            ...
            #print(bat_targets.shape, bat_masks.shape, bat_salency_targets.shape )
            with tf.GradientTape(persistent=False) as tape:
                ...
                seg_ce_loss = self.__calculate_seg_ce_loss( bat_masks, bat_seg_scores, skel_targets)
                ...
                grad = tape.gradient(ske_loss + 2.0*seg_ce_loss + 4.0*seg_dice_loss+2.0*reg_loss, self.model.trainable_variables, unconnected_gradients=tf.UnconnectedGradients.ZERO)

...
class Trainer_SSLIS(BaseTrainer):
    def __init__(self, model, config, use_dice=True, pertubations = None):
        super(Trainer_SSLIS, self).__init__(model, config)    
    ...
    def start_train(self, train_dataset, test_dataset, unlabeled_dataset, save_model_dir, pretrained_file=None):
        ...
        with self.summary_writer.as_default(): 
            ...
            for self.epoch in range(self.cfg.EPOCHS):
                ...
                tf.summary.scalar('train total loss', (agv_ske_loss+agv_seg_ce_loss+agv_seg_dice_loss+agv_reg_loss)*0.25, step = (self.epoch+1))
                ...
                tf.summary.scalar('eval total loss', (eval_ske_loss+eval_seg_ce_loss+eval_seg_dice_loss+eval_reg_loss)*0.25, step = (self.epoch+1))
    ...
    def __train_epoch(self, train_dataset, unlabeled_dataset, optimizer):
        ...
        for step in range(self.cfg.STEPS_PER_EPOCH):           
            ...
            with tf.GradientTape(persistent=False) as tape:
                ...
                student_bat_unlabeled_images  = self.__pertubation(bat_unlabeled_images.copy())
                feed_bat_ims = np.concatenate((bat_images, student_bat_unlabeled_images), axis=0)
                ...
                if self.epoch >= int(self.cfg.PSEUDOLABEL_WARMUP):
                    ...
                    bat_unlabeled_images = self.__pertubation(bat_unlabeled_images.copy())
                    bat_pseudo_ske_scores, bat_pseudo_seg_scores, bat_pseudo_reg_deltas = self.model(self.__cvt2_feed_format(bat_unlabeled_images))
                    ...
                    bat_unlabeled_ske_scores = bat_ske_scores[self.cfg.BATCH_SIZE:, ...]#[1, H, W, 1]
                    bat_unlabeled_seg_scores = bat_seg_scores[self.cfg.BATCH_SIZE:, ...]#[1, H, W, 26]
                    bat_unlabeled_reg_deltas = bat_reg_deltas[self.cfg.BATCH_SIZE:, ...]#[1, H, W, 1, 5]

                    pseudo_loss = self.__calculate_pseudo_loss(bat_unlabeled_ske_scores, \
                                                           bat_unlabeled_seg_scores,\
                                                           bat_unlabeled_reg_deltas,\

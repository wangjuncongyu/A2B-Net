...
class Trainer_SSL_CPS(BaseTrainer):
    def __init__(self, model, model_t, config, use_dice=True, pertubations = None):
        super(Trainer_SSL_CPS, self).__init__(model, config)    
    ...
    def start_train(self, train_dataset, test_dataset, unlabeled_dataset, save_model_dir, pretrained_file=None):
        ...
        self.model_t.build(input_shape = tuple([self.cfg.BATCH_SIZE]+self.cfg.INPUT_SHAPE + [3]))
        ...
        lr_schedule_t =  tf.keras.optimizers.schedules.ExponentialDecay(self.cfg.LR, self.cfg.DECAY_STEPS,\
                                                                      self.cfg.DECAY_RATE, staircase = True)
        ...
        with self.summary_writer.as_default(): 
            ...
            for self.epoch in range(self.cfg.EPOCHS):
                ...
                #training an epoch
                t1 = time.time()
                agv_ske_loss, agv_seg_ce_loss, agv_seg_dice_loss, agv_reg_loss, agv_pseudo_loss = self.__train_epoch(train_dataset, unlabeled_dataset, optimizer, optimizer_t)
                t2 = time.time()
    ...
    def __train_epoch(self, train_dataset, unlabeled_dataset, optimizer, optimizer_t):
        '''
        ...
        for step in range(self.cfg.STEPS_PER_EPOCH):           
            ...
            with tf.GradientTape() as tape, tf.GradientTape() as tape_t:
                bat_aug_unlabeled_images = self.__strong_pertubation(bat_unlabeled_images.copy())
                feed_bat_ims = np.concatenate((bat_images, bat_aug_unlabeled_images), axis=0)
                ...
                #print(ske_loss, seg_ce_loss, seg_dice_loss,reg_loss)
                model_loss = ske_loss + 2.0*seg_ce_loss + 4.0*seg_dice_loss+2.0*reg_loss
                if self.epoch >= int(self.cfg.PSEUDOLABEL_WARMUP):
                    bat_aug_unlabeled_images = self.__strong_pertubation(bat_unlabeled_images.copy())
                    feed_bat_ims = np.concatenate((bat_images, bat_aug_unlabeled_images), axis=0)
                    bat_t_ske_scores, bat_t_seg_scores, bat_t_reg_deltas = self.model_t(self.__cvt2_feed_format(feed_bat_ims))

                    ske_t_loss = self.__calculate_ske_loss(bat_salency_targets, bat_t_ske_scores[0:self.cfg.BATCH_SIZE,...])
                   
                    skel_targets = bat_salency_targets
                    seg_t_ce_loss = self.__calculate_seg_ce_loss( bat_masks, bat_t_seg_scores[0:self.cfg.BATCH_SIZE,...], skel_targets)
                
                    seg_t_dice_loss = self.__calculate_seg_dice_loss(bat_masks, bat_t_seg_scores[0:self.cfg.BATCH_SIZE,...]) if self.use_dice  else 0.0
                
                    reg_t_loss = self.__calculate_reg_loss(bat_anchor_locs,\
                                                     bat_anchors,\
                                                     bat_anchor_labels,\
                                                     bat_anchor_reg_rbboxs,\
                                                     bat_t_reg_deltas[0:self.cfg.BATCH_SIZE,...],\
                                                     bat_anchor_weights)
                
                    model_t_loss = ske_t_loss + 2.0*seg_t_ce_loss + 4.0*seg_t_dice_loss+2.0*reg_t_loss

                    bat_ske_scores = tf.stop_gradient(bat_ske_scores)
                    bat_seg_scores = tf.stop_gradient(bat_seg_scores)
                    bat_reg_deltas = tf.stop_gradient(bat_reg_deltas)

                    bat_t_ske_scores = tf.stop_gradient(bat_t_ske_scores)
                    bat_t_seg_scores = tf.stop_gradient(bat_t_seg_scores)
                    bat_t_reg_deltas = tf.stop_gradient(bat_t_reg_deltas)
                    ...
                    pseudo_loss = self.__calculate_pseudo_loss(bat_ske_scores, \
                                                           ...
                                                           bat_t_seg_scores,\
                                                           bat_t_reg_deltas)
                    pseudo_t_loss = self.__calculate_pseudo_loss(bat_t_ske_scores, \
                                                           bat_t_seg_scores,\
                                                           bat_t_reg_deltas,\
                                                           bat_ske_scores,\
                                                           bat_seg_scores,\
                    ...
                    model_t_loss += pseudo_t_loss
                else:
                    pseudo_loss = 0.0

                losses['pseudo_loss'].append(pseudo_loss)  

                grad = tape.gradient(model_loss, self.model.trainable_variables)                   
                optimizer.apply_gradients(grads_and_vars=zip(grad, self.model.trainable_variables))

                grad_t = tape_t.gradient(model_t_loss, self.model_t.trainable_variables)                   
                optimizer_t.apply_gradients(grads_and_vars=zip(grad_t, self.model_t.trainable_variables))
    ...
    def __strong_pertubation(self, images):
        #import imageio
        for i in range(images.shape[0]):
            im = images[i,...]
            im ,_ ,_ = self.pertubations(im, None, None)
            im = np.clip(im, 0.0, 255.0)
            #imageio.imsave('pertub.png', im)
            images[i,...] = im
        return images
    ...
    def __calculate_pseudo_loss(self, ske_scores, seg_scores,  reg_deltas, pseudo_ske_scores, pseudo_seg_scores, pseudo_reg_deltas):       
        ...
        max_prob = tf.reduce_max(pseudo_seg_scores, axis=-1)
        keep = tf.where(max_prob>0.6)
        if len(keep)<100:
            ce_loss = 0.0
        else:
            seg_scores = tf.gather_nd(seg_scores, keep)
            pseudo_seg_scores = tf.gather_nd(pseudo_seg_scores, keep)
            pseduo_labels = tf.argmax(pseudo_seg_scores, axis=-1)
            ce_loss = tf.keras.losses.SparseCategoricalCrossentropy()(pseduo_labels, seg_scores)            
        ...
        if len(keep)>100:
            ...
            student_map = tf.keras.layers.Concatenate(axis=-1)([ske_scores, reg_deltas])
            teacher_map = tf.keras.layers.Concatenate(axis=-1)([pseudo_ske_scores, pseudo_reg_deltas])
            mse_loss = tf.keras.losses.MeanSquaredError()(teacher_map, student_map)

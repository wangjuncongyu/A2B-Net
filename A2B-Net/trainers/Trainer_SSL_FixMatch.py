...
class Trainer_SSL_FixMatch(BaseTrainer):
    def __init__(self, model, config, use_dice=True, pertubations = None):
        super(Trainer_SSL_FixMatch, self).__init__(model, config)    
    ...
    def __train_epoch(self, train_dataset, unlabeled_dataset, optimizer):
        ...
        for step in range(self.cfg.STEPS_PER_EPOCH):           
            ...
            with tf.GradientTape(persistent=False) as tape:
                bat_unlabeled_images_strong_aug = self.__strong_pertubation(bat_unlabeled_images.copy())

                feed_bat_ims = np.concatenate((bat_images, bat_unlabeled_images, bat_unlabeled_images_strong_aug), axis=0)
                ...
                if self.epoch >= int(self.cfg.PSEUDOLABEL_WARMUP):
                    ...
                    bat_strong_aug_ske_scores = bat_ske_scores[self.cfg.BATCH_SIZE+self.cfg.UNLABEL_BATCH_SIZE:,...]
                    bat_strong_aug_seg_scores = bat_seg_scores[self.cfg.BATCH_SIZE+self.cfg.UNLABEL_BATCH_SIZE:,...]
                    bat_strong_aug_reg_deltas = bat_reg_deltas[self.cfg.BATCH_SIZE+self.cfg.UNLABEL_BATCH_SIZE:,...]

                    bat_pseudo_ske_scores = bat_ske_scores[self.cfg.BATCH_SIZE:self.cfg.BATCH_SIZE+self.cfg.UNLABEL_BATCH_SIZE,...]
                    bat_pseudo_seg_scores = bat_seg_scores[self.cfg.BATCH_SIZE:self.cfg.BATCH_SIZE+self.cfg.UNLABEL_BATCH_SIZE,...]
                    bat_pseudo_reg_deltas = bat_reg_deltas[self.cfg.BATCH_SIZE:self.cfg.BATCH_SIZE+self.cfg.UNLABEL_BATCH_SIZE,...]
                   

                    pseudo_loss = self.__calculate_pseudo_loss(bat_strong_aug_ske_scores, \
                                                           bat_strong_aug_seg_scores,\
                                                           bat_strong_aug_reg_deltas,\

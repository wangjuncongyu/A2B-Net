...
CFGS = {
    'swin_tiny_512': dict(input_size=(512, 512), window_size=4, embed_dim=96, depths=[2, 2, 6, 2], num_heads=[3, 6, 12, 24]),
    'swin_small_512': dict(input_size=(512, 512), window_size=4, embed_dim=96, depths=[2, 2, 18, 2], num_heads=[3, 6, 12, 24]),
    'swin_base_512': dict(input_size=(512, 512), window_size=4, embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32]),
    'swin_large_512': dict(input_size=(512, 512), window_size=4, embed_dim=192, depths=[2, 2, 18, 2], num_heads=[6, 12, 24, 48])
}
...
class SwinTransformerModel(tf.keras.Model):
    def __init__(self, model_name='swin_tiny_patch4_window7_224', include_top=False,
                 img_size=(224, 224), patch_size=(2, 2), in_chans=3, num_classes=1000,
        ...
        #lateral layers
        self.L1 = tf.keras.layers.Conv2D(256, (1, 1), (1, 1), padding="same")
        self.L2 = tf.keras.layers.Conv2D(256, (1, 1), (1, 1), padding="same")
        self.L3 = tf.keras.layers.Conv2D(256, (1, 1), (1, 1), padding="same")
        self.L4 = tf.keras.layers.Conv2D(256, (1, 1), (1, 1), padding="same")
        
        #smooth layers     
        self.S1 = tf.keras.layers.Conv2D(256, (3, 3), (1, 1),  padding="same")  
        self.S2 = tf.keras.layers.Conv2D(256, (3, 3), (1, 1),  padding="same")
        self.S3 = tf.keras.layers.Conv2D(256, (3, 3), (1, 1),  padding="same")
    ...
    def forward_features(self, x):
        ...
        for i_layer in range(self.num_layers):
            ...
            self.Cs.append(x)
        C1, C2, C3, C4 = self.Cs
        C0 = tf.reshape(C0, (-1, H//2, W//2, C0.shape[-1]))
        C1 = tf.reshape(C1, (-1, H//4, W//4, C1.shape[-1]))
        C2 = tf.reshape(C2, (-1, H//8, W//8, C2.shape[-1]))
        C3 = tf.reshape(C3, (-1, H//16, W//16, C3.shape[-1]))
        #C4 = tf.reshape(C4, (-1, H//16, W//16, C4.shape[-1]))
        
        #top-->down
        P3 = self.L1(C3)
        P2 = self.S3(self._upsample_add(P3, self.L2(C2)))#s8
        P1 = self.S2(self._upsample_add(P2, self.L3(C1)))#s4
        P0 = self.S1(self._upsample_add(P1, self.L4(C0)))#s2       
...
def SwinTransformer(model_name='swin_tiny_512', num_classes=1000, include_top=False, pretrained=False, use_tpu=False, cfgs=CFGS):
    cfg = cfgs[model_name]

...
import time
pred_bboxes = np.array([[10, 20, 30, 40, 15]])
pred_bboxes = np.repeat(pred_bboxes, 40000, axis=0)

gt_bboxes = np.array([[10, 23, 28, 38, 21]])
gt_bboxes = np.repeat(gt_bboxes, 42, axis=0)
t1 = time.time()
ious = rotate_iou_gpu_eval(pred_bboxes, gt_bboxes)
t2 = time.time()
print('times:', t2-t1)
